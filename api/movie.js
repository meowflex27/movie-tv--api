// /pages/api/movie.js

export default async function handler(req, res) {
  const { id } = req.query;

  if (!id) {
    return res.status(400).json({ error: "Missing ?id (format: movieId)" });
  }

  try {
    // 🎬 Fetch video data from external server
    const fetchUrl = `http://213.232.235.66:3014/movie/${id}`;
    const response = await fetch(fetchUrl);
    const result = await response.json();

    if (result.code !== 0 || !result.data?.downloads) {
      return res.status(404).json({ error: "Movie not found or invalid response." });
    }

    // 🎞️ Get movie title from TMDB
    const tmdbApiKey = 'ea97a714a43a0e3481592c37d2c7178a';
    const tmdbRes = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${tmdbApiKey}`);
    const tmdbData = await tmdbRes.json();

    const rawTitle = tmdbData.title || `Movie_${id}`;
    const safeTitle = rawTitle.replace(/[^a-z0-9]/gi, '_');

    // 📥 Format download links using proxy
    const downloads = result.data.downloads.map(d => {
      const encodedOriginalUrl = encodeURIComponent(d.url);
      return {
        resolution: d.resolution,
        size: (parseInt(d.size) / 1024 / 1024).toFixed(1), // MB
        url: `https://meowflex.com/proxy.php?url=${encodedOriginalUrl}&n=${encodeURIComponent(safeTitle)}.mp4`
      };
    });

    // 🌍 Format subtitles
    const subtitles = (result.data.captions || []).map(c => ({
      lanName: c.lanName,
      size: (parseInt(c.size) / 1024).toFixed(1), // KB
      url: c.url
    }));

    // ✅ Final JSON response
    res.status(200).json({
      id,
      title: rawTitle,
      downloads,
      subtitles
    });

  } catch (err) {
    console.error("❌ Error fetching movie data:", err);
    res.status(500).json({ error: "Failed to fetch movie data." });
  }
}
